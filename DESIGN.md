...
### use case:
    1) someone in the world wants to play a game they are good and win real money from it
    ...
    3) the host initializes the 'game' 
        ...
        - 2) creates a 'game' on our smart contract (our contract is an ERC20 token based smart contract and is called "GTA" = Gamer Token Award)
                - when creating the game...
                    ...
                    the host may also choose to set a 'host_fee' for being the 'host' (optiional)
    ...
    6) playeers play the game
        - players then play the game like normal, on the COD platform (nothign to do with our GTA smart contract)
    7) paying out winners
        ...
        - 3) the GTA contract then automaticall pays the winners
...
### maintaing the market
    1) when we launch the contract, we also create the intial liquidity pools on the dexes (pulseX, 9inch, uniswap, etc.)
        NOTE: we don't need to put up a lot of money at all, because liquidity will be added from each game's 'entry_fees'
    2) Q: whats the initial price?
    ...
    3) Q:can winners cash out right away
        ...
        NOTE: the price won't drop because the exact value from the 'entry_fees' will be added to the dexes
